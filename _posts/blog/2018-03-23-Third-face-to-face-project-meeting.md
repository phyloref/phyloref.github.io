...
In terms of project organization, we discussed mainly three issues. We planed for the upcoming Advisory Board meeting on Apr 23. We will provide a series of 70-min long presentations, given by the project PIs as well as the two postdocs, to get the advisors up to speed with what we are doing.

## Pending issues (aka the questions that keep Hilmar up at night)
(a) How are going to solve specifier matching?
See below for more about the scale of the problem.
(b) What are we going to produce that draws people in?
See below for a full discussion.

Specifier matching - What is the relationship between C and A (Hilmar's example) 
We need a huge knowledgebase of phylogenetic relationships between labels, which would allow us to answer the question: can we or can we not replace a node
We might have to do a whole lot of recursive querying
We don’t need to know detailed relationships: we just need to know whether the clade of interest is split between different phylogenies
If we can show that we can use this via the Open Tree of Life, that saves us a lot of work AND makes a powerful case for having the Open Tree of Life
